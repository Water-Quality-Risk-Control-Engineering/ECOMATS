#!/usr/bin/env python3
"""
ECOMATS - 基于CrewAI的水处理材料设计多智能体系统 / ECOMATS - Multi-Agent System for Water Treatment Material Design Based on CrewAI
"""
            ...
            if "results" in result_data and isinstance(result_data["results"], list):
                for item in result_data["results"]:
                    if "recommendations" in item:
                        feedback += f"改进建议: {item['recommendations']}\n / Improvement suggestions: {item['recommendations']}\n"
                    if "cons" in item:
                        feedback += f"存在的问题: {item['cons']}\n / Existing issues: {item['cons']}\n"
...
def check_if_iteration_needed(result):
    ...
    try:
        ...
        print(f"检查迭代需求时出错: {e} / Error checking iteration needs: {e}")
        return False
...
def run_design_iteration(user_requirement, llm, iteration_count=0):
    ...
    if iteration_count >= Config.MAX_DESIGN_ITERATIONS:
        return "已达到最大迭代次数，停止迭代设计。/ Maximum iteration count reached, stopping iterative design."
    
    print(f"开始第 {iteration_count + 1} 轮设计迭代... / Starting round {iteration_count + 1} of design iteration...")
    ...
    if check_if_iteration_needed(result):
        print("当前设计方案未达到要求，需要进行迭代优化... / Current design solution does not meet requirements, iteration optimization is needed...")
        ...
        if feedback:
            ...
            updated_requirement = f"{user_requirement}\n\n基于上一轮评估的改进建议：{feedback} / Improvement suggestions based on last round evaluation: {feedback}"
            print(f"改进建议: {feedback} / Improvement suggestions: {feedback}")
            # 进行下一轮迭代 / Proceed to next iteration
            return run_design_iteration(updated_requirement, llm, iteration_count + 1)
        else:
            return result
    else:
        print("设计方案已达到要求，无需进一步迭代。/ Design solution meets requirements, no further iteration needed.")
        return result

def run_preset_workflow(user_requirement, llm):
    """运行预设工作流模式 / Run preset workflow mode"""
    print("启动预设工作流模式... / Starting preset workflow mode...")
...
def run_autonomous_workflow(user_requirement, llm):
    ...
    print("启动智能体自主调度模式... / Starting agent autonomous scheduling mode...")
